...
/**
 * Sends the forgot-password OTP email through Brevo's transactional email
 * REST API. Calls the HTTP API directly with the JDK's built-in HttpClient
 * (Java 11+) rather than pulling in a mail/SDK dependency — this project
 * already hand-builds small JSON payloads elsewhere (e.g.
 * PatientSearchServlet) instead of adding a library for it.
 *
...
public final class BrevoMailer {
    ...
    public static void sendOtpEmail(String toEmail, String toName, String otp) throws IOException, InterruptedException {
        ...
        String subject = "Your Sunrise Dental password reset code";
