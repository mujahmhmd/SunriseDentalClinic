...
<head>
  ...
  <title>Services - Sunrise Dental</title>
...
<body class="h-full overflow-hidden flex bg-clinic-50 text-clinic-900">
      ...
      <div class="bg-white rounded-2xl border border-clinic-100 shadow-sm overflow-hidden">
        ...
        <div class="p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-clinic-100">
          <div class="relative w-full sm:max-w-xs">
            ...
            <input type="text" id="serviceSearch" value="<%= searchQuery %>" placeholder="Search by service name"
        ...
        <div id="serviceTableContainer">
          <jsp:include page="components/service-table.jsp" />
        </div>
      </div>
    </main>
  </div>
  ...
  <jsp:include page="components/confirm-modal.jsp">
    <jsp:param name="id" value="deleteServiceModal" />
    <jsp:param name="title" value="Remove this service?" />
    <jsp:param name="message" value="This will permanently remove it from the catalog. This can't be undone." />
  ...
  <script>
    (function () {
      ...
      var container = document.getElementById('serviceTableContainer');
      var debounceTimer = null;
      var currentPage = <%= initialPage %>;

      function loadServices(query, page, pushState) {
        var url = 'services?q=' + encodeURIComponent(query) + '&page=' + page;
        fetch(url + '&ajax=1')
      ...
      container.addEventListener('click', function (e) {
        ...
        if (link) {
          ...
          e.preventDefault();
          loadServices(searchInput.value, link.dataset.page, true);
          return;
        }

        var toggle = e.target.closest('.status-toggle');
        if (toggle) {
          fetch('toggleServiceStatus?id=' + toggle.dataset.id, { method: 'POST' })
            .then(function () {
              loadServices(searchInput.value, currentPage, false);
      ...
      window.addEventListener('popstate', function (e) {
        ...
        loadServices(state.q, state.page, false);
      });
    })();

    // Called from the Delete button rendered inside the service table fragment.
    function confirmDeleteService(id, name) {
      document.querySelector('#deleteServiceModal p').textContent =
        'This will permanently remove “' + name + '” from the catalog. This can’t be undone.';
      document.querySelector('#deleteServiceModal button.bg-coral-500').onclick = function () {
        window.location.href = 'deleteService?id=' + id;
      };
      openConfirmModal('deleteServiceModal');
    }
