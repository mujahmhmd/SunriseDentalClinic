...
<head>
  ...
  <title>Appointment Receipt - Sunrise Dental</title>
...
<body class="min-h-screen bg-clinic-50 text-clinic-900 py-10 px-4">
  ...
  <div class="max-w-xl mx-auto">
    ...
    <% if (request.getParameter("justBooked") != null) { %>
    ...
    <div class="receipt-card bg-white rounded-2xl border border-clinic-100 shadow-sm p-8">
      ...
      <div class="flex items-center gap-3 pb-6 mb-6 border-b border-dashed border-clinic-100">
        <div class="shrink-0 w-11 h-11 rounded-xl bg-gradient-to-br from-clinic-600 to-clinic-800 flex items-center justify-center">
        ...
        <div>
          <p class="font-display text-base leading-tight text-clinic-900">Sunrise Dental Clinic</p>
          <p class="text-xs text-clinic-700/50">Appointment Receipt</p>
        </div>
        <div class="ml-auto text-right">
          <p class="text-xs text-clinic-700/50">Appointment No.</p>
          <p class="font-display text-base text-clinic-900"><%= request.getAttribute("appointmentNumber") %></p>
        </div>
      </div>

      <div class="grid grid-cols-2 gap-x-6 gap-y-5 mb-6">
        <div>
          <p class="text-xs text-clinic-700/50 mb-0.5">Patient</p>
          <p class="text-sm font-medium text-clinic-900"><%= request.getAttribute("patientName") %></p>
          <p class="text-sm text-clinic-700/60"><%= request.getAttribute("patientPhone") %></p>
        </div>
        <div>
          <p class="text-xs text-clinic-700/50 mb-0.5">Doctor</p>
          <p class="text-sm font-medium text-clinic-900">Dr. <%= request.getAttribute("doctorName") %></p>
          <% if (request.getAttribute("qualifications") != null) { %>
          <p class="text-sm text-clinic-700/60"><%= request.getAttribute("qualifications") %></p>
          <% } %>
        </div>
        <div>
          <p class="text-xs text-clinic-700/50 mb-0.5">Date</p>
          <p class="text-sm font-medium text-clinic-900"><%= request.getAttribute("date") %></p>
        </div>
        <div>
          <p class="text-xs text-clinic-700/50 mb-0.5">Time</p>
          <p class="text-sm font-medium text-clinic-900"><%= request.getAttribute("time") %></p>
        </div>
        <div class="col-span-2">
          <p class="text-xs text-clinic-700/50 mb-0.5">Reason for Visit</p>
          <p class="text-sm font-medium text-clinic-900"><%= request.getAttribute("reason") %></p>
        </div>
      </div>
