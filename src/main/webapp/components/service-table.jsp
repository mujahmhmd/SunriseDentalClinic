<%--
  Service count + table + pagination. Split out from services.jsp so ServiceServlet
  can render just this fragment (no sidebar/header/html shell) for the AJAX
  live-search/pagination requests, while services.jsp includes the same
  fragment for a normal full-page load. Reads request attributes set by
  ServiceServlet: serviceList, totalCount, totalPages, currentPage, pageSize,
  searchQuery.
--%>
<%@ page contentType="text/html;charset=UTF-8" pageEncoding="UTF-8" buffer="64kb" %>
<%@ page import="java.util.List, java.util.Map, java.net.URLEncoder, java.text.DecimalFormat" %>
<%
  List<Map<String, String>> serviceList = (List<Map<String, String>>) request.getAttribute("serviceList");
  ...
  int rangeStart = serviceList == null || serviceList.isEmpty() ? 0 : (currentPage - 1) * pageSize + 1;
  int rangeEnd = serviceList == null || serviceList.isEmpty() ? 0 : rangeStart + serviceList.size() - 1;
%>
<!-- Table -->
<div class="overflow-x-auto px-6 pt-3">
  <table class="w-full text-sm">
    ...
    <tbody class="divide-y divide-clinic-50">
      <% if (serviceList != null) { for (Map<String, String> service : serviceList) {
           String id = service.get("id");
           String description = service.get("description");
           boolean active = "active".equals(service.get("status"));
      %>
      <tr class="hover:bg-clinic-50/60 transition-colors">
        <td class="py-3.5 pr-4 font-medium text-clinic-900"><%= service.get("name") %></td>
        <td class="py-3.5 pr-4 text-clinic-700/60 max-w-xs truncate"><%= description == null ? "" : description %></td>
        <td class="py-3.5 pr-4 text-clinic-700/70">Rs. <%= priceFormat.format(Double.parseDouble(service.get("price"))) %></td>
        ...
        <td class="py-3.5 pr-4">
          <div class="flex items-center justify-end gap-1">
            <a href="editService?id=<%= id %>" aria-label="Edit"
            ...
            <button type="button" aria-label="Delete" onclick="confirmDeleteService('<%= id %>', '<%= service.get("name") %>')"
  ...
  <div class="py-14 text-center">
    <p class="text-clinic-700/60 text-sm">No services match your search.</p>
  </div>
  <% } %>
</div>
...
<div class="px-6 py-4 border-t border-clinic-100 flex flex-wrap items-center justify-between gap-3">
  <p class="text-sm text-clinic-700/60">
    <% if (totalCount == 0) { %>
      No services found<%= searchQuery.isEmpty() ? "" : " for “" + searchQuery + "”" %>.
    <% } else { %>
      Showing <span class="font-medium text-clinic-900"><%= rangeStart %>&ndash;<%= rangeEnd %></span>
      of <span class="font-medium text-clinic-900"><%= totalCount %></span> service<%= totalCount == 1 ? "" : "s" %>
    <% } %>
  </p>

  <% if (totalCount > 0) { %>
  <div class="flex items-center gap-1">
    <a href="services?q=<%= encodedQuery %>&page=<%= Math.max(1, currentPage - 1) %>" data-page="<%= Math.max(1, currentPage - 1) %>"
    ...
    <% for (int p = 1; p <= totalPages; p++) { %>
      <a href="services?q=<%= encodedQuery %>&page=<%= p %>" data-page="<%= p %>"
    ...
    <a href="services?q=<%= encodedQuery %>&page=<%= Math.min(totalPages, currentPage + 1) %>" data-page="<%= Math.min(totalPages, currentPage + 1) %>"
