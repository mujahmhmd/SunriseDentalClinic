<%--
  ...
  doctors, patients, appointments, reports, billing) so the matching nav item
  gets highlighted.
--%>
<%@ page buffer="64kb" %>
<%
...
<aside id="sidebar" class="sidebar relative flex flex-col shrink-0 bg-gradient-to-b from-clinic-900 to-clinic-800 text-clinic-50">
  ...
  <nav class="flex-1 px-3.5 py-6 space-y-1 overflow-y-auto overflow-x-hidden">
    ...
    <a href="patients.jsp" class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors <%= "patients".equals(activePage) ? "active bg-white/10 text-white" : "text-clinic-50/65 hover:bg-white/5 hover:text-white" %>">
      <svg class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
