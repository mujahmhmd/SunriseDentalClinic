        ...
        <td class="py-3.5 pr-4">
          <div class="flex items-center justify-end gap-1">
            ...
            <% if (scheduled || processingPayment) {
                 boolean canComplete = Boolean.parseBoolean(appt.get("canComplete"));
                 String completeLabel = processingPayment ? "Resume payment" : "Complete & bill";
                 String completeTitle = canComplete ? completeLabel
                     : "Can't complete yet - scheduled for " + appt.get("date") + ". Available on or after that date.";
            %>
            <button type="button"
                    class="complete-appointment p-2 rounded-lg transition-colors <%= canComplete
                        ? "text-clinic-700/60 hover:bg-clinic-600/10 hover:text-clinic-700"
                        : "text-clinic-700/25 cursor-not-allowed" %>"
                    data-id="<%= id %>" <%= canComplete ? "" : "disabled" %>
                    aria-label="<%= completeTitle %>" title="<%= completeTitle %>">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
