<%--
  Patient count + table + pagination. Split out from patients.jsp so
  PatientServlet can render just this fragment (no sidebar/header/html shell)
  for the AJAX live-search/pagination requests, while patients.jsp includes
  the same fragment for a normal full-page load. Reads request attributes
  set by PatientServlet: patientList, totalCount, totalPages, currentPage,
...
<%
  List<Map<String, String>> patientList = (List<Map<String, String>>) request.getAttribute("patientList");
  ...
  int rangeStart = patientList == null || patientList.isEmpty() ? 0 : (currentPage - 1) * pageSize + 1;
  int rangeEnd = patientList == null || patientList.isEmpty() ? 0 : rangeStart + patientList.size() - 1;
%>
<!-- Table -->
<div class="overflow-x-auto px-6 pt-3">
  <table class="w-full text-sm">
    <thead>
      <tr class="text-left text-clinic-700/50 text-xs uppercase tracking-wide">
        ...
        <th class="py-3 pr-4 font-medium">NIC</th>
        <th class="py-3 pr-4 font-medium text-right">Actions</th>
      </tr>
    </thead>
    <tbody class="divide-y divide-clinic-50">
      <% if (patientList != null) { for (Map<String, String> patient : patientList) {
           String id = patient.get("id");
           String nic = patient.get("nic");
      ...
      <tr class="hover:bg-clinic-50/60 transition-colors">
        ...
        <td class="py-3.5 pr-4 text-clinic-700/60"><%= patient.get("phone") %></td>
        ...
        <td class="py-3.5 pr-4">
          <div class="flex items-center justify-end gap-1">
            ...
            <button type="button" aria-label="Delete" onclick="confirmDeletePatient('<%= id %>', '<%= patient.get("name") %>')"
  ...
  <div class="py-14 text-center">
    <p class="text-clinic-700/60 text-sm">No patients match your search.</p>
  </div>
  <% } %>
</div>
...
<div class="px-6 py-4 border-t border-clinic-100 flex flex-wrap items-center justify-between gap-3">
  <p class="text-sm text-clinic-700/60">
    <% if (totalCount == 0) { %>
      No patients found<%= searchQuery.isEmpty() ? "" : " for “" + searchQuery + "”" %>.
    <% } else { %>
      Showing <span class="font-medium text-clinic-900"><%= rangeStart %>&ndash;<%= rangeEnd %></span>
      of <span class="font-medium text-clinic-900"><%= totalCount %></span> patient<%= totalCount == 1 ? "" : "s" %>
    <% } %>
  </p>

  <% if (totalCount > 0) { %>
  <div class="flex items-center gap-1">
    <a href="patients?q=<%= encodedQuery %>&page=<%= Math.max(1, currentPage - 1) %>" data-page="<%= Math.max(1, currentPage - 1) %>"
    ...
    <% for (int p = 1; p <= totalPages; p++) { %>
      <a href="patients?q=<%= encodedQuery %>&page=<%= p %>" data-page="<%= p %>"
    ...
    <a href="patients?q=<%= encodedQuery %>&page=<%= Math.min(totalPages, currentPage + 1) %>" data-page="<%= Math.min(totalPages, currentPage + 1) %>"
