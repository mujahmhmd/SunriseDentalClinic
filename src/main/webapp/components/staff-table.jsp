<%--
  Staff count + table + pagination. Split out from staffs.jsp so StaffServlet
  can render just this fragment (no sidebar/header/html shell) for the AJAX
  live-search/pagination requests, while staffs.jsp includes the same fragment
  for a normal full-page load. Reads request attributes set by StaffServlet:
  staffList, totalCount, totalPages, currentPage, pageSize, searchQuery.
--%>
...
<%
  ...
  int rangeStart = staffList == null || staffList.isEmpty() ? 0 : (currentPage - 1) * pageSize + 1;
  int rangeEnd = staffList == null || staffList.isEmpty() ? 0 : rangeStart + staffList.size() - 1;
%>
<!-- Table -->
<div class="overflow-x-auto px-6 pt-3">
  <table class="w-full text-sm">
    ...
    <tbody class="divide-y divide-clinic-50">
      <% if (staffList != null) { for (Map<String, String> staff : staffList) {
           String id = staff.get("id");
           boolean active = "active".equals(staff.get("status"));
      %>
      <tr class="hover:bg-clinic-50/60 transition-colors">
        <td class="py-3.5 pr-4 font-medium text-clinic-900"><%= staff.get("name") %></td>
        <td class="py-3.5 pr-4 text-clinic-700/70"><%= staff.get("username") %></td>
        ...
        <td class="py-3.5 pr-4 text-clinic-700/60"><%= staff.get("joined") %></td>
        <td class="py-3.5 pr-4">
          <div class="flex items-center justify-end gap-1">
            <a href="editStaff?id=<%= id %>" aria-label="Edit"
            ...
            <button type="button" aria-label="Delete" onclick="confirmDeleteStaff('<%= id %>', '<%= staff.get("name") %>')"
  ...
  <div class="py-14 text-center">
    <p class="text-clinic-700/60 text-sm">No staff members match your search.</p>
  </div>
  <% } %>
</div>
...
<div class="px-6 py-4 border-t border-clinic-100 flex flex-wrap items-center justify-between gap-3">
  <p class="text-sm text-clinic-700/60">
    <% if (totalCount == 0) { %>
      No staff found<%= searchQuery.isEmpty() ? "" : " for “" + searchQuery + "”" %>.
    <% } else { %>
      Showing <span class="font-medium text-clinic-900"><%= rangeStart %>&ndash;<%= rangeEnd %></span>
      of <span class="font-medium text-clinic-900"><%= totalCount %></span> staff member<%= totalCount == 1 ? "" : "s" %>
    <% } %>
  </p>

  <% if (totalCount > 0) { %>
  <div class="flex items-center gap-1">
    <a href="staffs?q=<%= encodedQuery %>&page=<%= Math.max(1, currentPage - 1) %>" data-page="<%= Math.max(1, currentPage - 1) %>"
    ...
    <% for (int p = 1; p <= totalPages; p++) { %>
      <a href="staffs?q=<%= encodedQuery %>&page=<%= p %>" data-page="<%= p %>"
    ...
    <a href="staffs?q=<%= encodedQuery %>&page=<%= Math.min(totalPages, currentPage + 1) %>" data-page="<%= Math.min(totalPages, currentPage + 1) %>"
